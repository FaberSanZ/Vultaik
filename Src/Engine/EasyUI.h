...
public:
    ...
    virtual void generateCommands(std::vector<UIDrawCommand>& commands, int layer = 0)
    {
        ...
        auto col = toFloat4(m_color);
        float x0 = m_absX, y0 = m_absY, x1 = x0 + m_width, y1 = y0 + m_height;
        cmd.vertices.push_back({ {x0, y0, 0, 1}, {col[0], col[1], col[2], col[3]}, {0, 0} });
        cmd.vertices.push_back({ {x1, y0, 0, 1}, {col[0], col[1], col[2], col[3]}, {0, 0} });
        cmd.vertices.push_back({ {x1, y1, 0, 1}, {col[0], col[1], col[2], col[3]}, {0, 0} });
        cmd.vertices.push_back({ {x0, y1, 0, 1}, {col[0], col[1], col[2], col[3]}, {0, 0} });
...
protected:
    ...
    float m_desiredWidth, m_desiredHeight;
...
public:
    ...
    void generateVertices(std::vector<UIVertex>& v, std::vector<uint32_t>& i) override
    {
        ...
        float y0 = m_absY + (m_height - m_handleSize) * 0.5f, y1 = y0 + m_handleSize;
            ...
            auto addBorderRect = [&](float X0, float Y0, float X1, float Y1, float t)
                {
                    ...
                    v.push_back({ {X0, Y0, 0, 1}, {colB[0], colB[1], colB[2], colB[3]}, {0, 0} });
                    v.push_back({ {X1, Y0, 0, 1}, {colB[0], colB[1], colB[2], colB[3]}, {0, 0} });
                    v.push_back({ {X1, Y0 + t, 0, 1}, {colB[0], colB[1], colB[2], colB[3]}, {0, 0} });
                    v.push_back({ {X0, Y0 + t, 0, 1}, {colB[0], colB[1], colB[2], colB[3]}, {0, 0} });
                    i.push_back(bbase + 0); i.push_back(bbase + 1); i.push_back(bbase + 2);
                    i.push_back(bbase + 0); i.push_back(bbase + 2); i.push_back(bbase + 3);
                    // Similar para los otros tres lados (código acortado por brevedad, pero igual que antes)
                    // ... (puedes mantener el código completo de tu implementación)
                };
    ...
    void generateCommands(std::vector<UIDrawCommand>& commands, int layer) override
    {
        ...
        if (m_handleBorder)
        {
            // ... igual que en generateVertices pero usando cmd en lugar de v
        }
    }
...
public:
    ...
    void generateCommands(std::vector<UIDrawCommand>& commands, int layer) override
    {
        ...
        float padding = 4.0f;
        float x0 = m_absX + padding, y0 = m_absY + padding;
        float x1 = m_absX + m_width - padding, y1 = m_absY + m_height - padding;

        auto colFill = toFloat4(0xFF4CAF50);
        uint32_t baseFill = (uint32_t)fillCmd.vertices.size();
        fillCmd.vertices.push_back({ {x0, y0, 0, 1}, {colFill[0], colFill[1], colFill[2], colFill[3]}, {0, 0} });
        fillCmd.vertices.push_back({ {x1, y0, 0, 1}, {colFill[0], colFill[1], colFill[2], colFill[3]}, {0, 0} });
        fillCmd.vertices.push_back({ {x1, y1, 0, 1}, {colFill[0], colFill[1], colFill[2], colFill[3]}, {0, 0} });
        fillCmd.vertices.push_back({ {x0, y1, 0, 1}, {colFill[0], colFill[1], colFill[2], colFill[3]}, {0, 0} });
        ...
        float w = m_width - 2 * padding, h = m_height - 2 * padding;
        float cx = x0 + w * 0.5f, cy = y0 + h * 0.5f;

        float x1v = x0 + w * 0.2f, y1v = cy;
        float x2v = x0 + w * 0.45f, y2v = y0 + h * 0.7f;
        float x3v = x0 + w * 0.8f, y3v = y0 + h * 0.3f;
        ...
        auto addSegment = [&](float ax, float ay, float bx, float by)
            {
                ...
                markCmd.vertices.push_back({ {ax + perpX, ay + perpY, 0, 1}, {colWhite[0], colWhite[1], colWhite[2], colWhite[3]}, {0, 0} });
                markCmd.vertices.push_back({ {ax - perpX, ay - perpY, 0, 1}, {colWhite[0], colWhite[1], colWhite[2], colWhite[3]}, {0, 0} });
                markCmd.vertices.push_back({ {bx - perpX, by - perpY, 0, 1}, {colWhite[0], colWhite[1], colWhite[2], colWhite[3]}, {0, 0} });
                markCmd.vertices.push_back({ {bx + perpX, by + perpY, 0, 1}, {colWhite[0], colWhite[1], colWhite[2], colWhite[3]}, {0, 0} });
...
public:
    ...
    void addOption(const std::string& text) { m_options.push_back(text); if (m_selectedIndex == -1 && m_options.size() == 1) setSelectedIndex(0); }
    ...
    std::string getSelectedText() const { return (m_selectedIndex >= 0 && m_selectedIndex < (int)m_options.size()) ? m_options[m_selectedIndex] : ""; }
    ...
    void onEvent(UIEvent& e) override
    {
        if (e.type == UIEventType::MouseDown)
        {
            if (e.x >= m_absX && e.x < m_absX + m_width && e.y >= m_absY && e.y < m_absY + m_height)
            {
                toggle();
                e.handled = true;
                return;
            ...
            if (m_isOpen)
            {
                ...
                if (e.x >= panelX && e.x < panelX + panelW && e.y >= panelY && e.y < panelY + panelH)
                {
    ...
    void generateCommands(std::vector<UIDrawCommand>& commands, int layer) override
    {
        ...
        arrowCmd.vertices.push_back({ {arrowX, arrowY, 0, 1}, {colArrow[0], colArrow[1], colArrow[2], colArrow[3]}, {0, 0} });
        arrowCmd.vertices.push_back({ {arrowX + arrowSize, arrowY, 0, 1}, {colArrow[0], colArrow[1], colArrow[2], colArrow[3]}, {0, 0} });
        arrowCmd.vertices.push_back({ {arrowX + arrowSize * 0.5f, arrowY + arrowSize, 0, 1}, {colArrow[0], colArrow[1], colArrow[2], colArrow[3]}, {0, 0} });
        ...
        panelCmd.vertices.push_back({ {panelX, panelY, 0, 1}, {colBg[0], colBg[1], colBg[2], colBg[3]}, {0, 0} });
        panelCmd.vertices.push_back({ {panelX + panelW, panelY, 0, 1}, {colBg[0], colBg[1], colBg[2], colBg[3]}, {0, 0} });
        panelCmd.vertices.push_back({ {panelX + panelW, panelY + panelH, 0, 1}, {colBg[0], colBg[1], colBg[2], colBg[3]}, {0, 0} });
        panelCmd.vertices.push_back({ {panelX, panelY + panelH, 0, 1}, {colBg[0], colBg[1], colBg[2], colBg[3]}, {0, 0} });
        ...
        auto addBorder = [&](float X0, float Y0, float X1, float Y1) {
            ...
            panelCmd.vertices.push_back({ {X0, Y0, 0, 1}, {colBorder[0], colBorder[1], colBorder[2], colBorder[3]}, {0, 0} });
            panelCmd.vertices.push_back({ {X1, Y0, 0, 1}, {colBorder[0], colBorder[1], colBorder[2], colBorder[3]}, {0, 0} });
            panelCmd.vertices.push_back({ {X1, Y1, 0, 1}, {colBorder[0], colBorder[1], colBorder[2], colBorder[3]}, {0, 0} });
            panelCmd.vertices.push_back({ {X0, Y1, 0, 1}, {colBorder[0], colBorder[1], colBorder[2], colBorder[3]}, {0, 0} });
        ...
        for (int i = 0; i < visibleCount; ++i)
        {
            ...
            panelCmd.vertices.push_back({ {panelX + 1, optY + 1, 0, 1}, {colOpt[0], colOpt[1], colOpt[2], colOpt[3]}, {0, 0} });
            panelCmd.vertices.push_back({ {panelX + panelW - 1, optY + 1, 0, 1}, {colOpt[0], colOpt[1], colOpt[2], colOpt[3]}, {0, 0} });
            panelCmd.vertices.push_back({ {panelX + panelW - 1, optY + m_itemHeight - 1, 0, 1}, {colOpt[0], colOpt[1], colOpt[2], colOpt[3]}, {0, 0} });
            panelCmd.vertices.push_back({ {panelX + 1, optY + m_itemHeight - 1, 0, 1}, {colOpt[0], colOpt[1], colOpt[2], colOpt[3]}, {0, 0} });
...
public:
    ...
    void generateCommands(std::vector<UIDrawCommand>& commands, int layer) override
    {
        ...
        float x0 = m_absX, y0 = m_absY, x1 = x0 + fillWidth, y1 = y0 + m_height;
...
public:
    ...
    void setScreenSize(uint32_t width, uint32_t height) { m_screenWidth = width; m_screenHeight = height; }

    void updateLayout()
    {
        if (m_root) m_root->updateLayout(0, 0, (float)m_screenWidth, (float)m_screenHeight);
    }

    void render(std::vector<UIVertex>& v, std::vector<uint32_t>& i) { if (m_root) m_root->generateVertices(v, i); }
    void render(std::vector<UIDrawCommand>& commands) { if (m_root) m_root->generateCommands(commands, 0); }
